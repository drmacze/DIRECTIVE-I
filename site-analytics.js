(() => {
  ...
  if(!session||!session.id||now-(session.updatedAt||0)>30*60*1000){
    session={id:(crypto.randomUUID?.()||`${now}-${Math.random().toString(36).slice(2)}`).replace(/[^a-zA-Z0-9_-]/g,''),updatedAt:now};
  ...
  async function track(eventType='page_view',metadata={}){
    const payload={session_id:session.id,event_type:eventType,path:location.pathname.replace('/DIRECTIVE-I','')||'/',referrer,device,browser,os,screen_width:screen.width||null,metadata};
    try{await fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),keepalive:true})}catch(_){ }
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if(!target)return;
    if(target.matches('[data-open-trailer]'))track('trailer_open');
    const href=target.getAttribute?.('href')||'';
    if(/showcase\.html/.test(href))track('showcase_open');
    else if(/documentation\.html/.test(href))track('documentation_open');
    else if(/core-gameplay\.html/.test(href))track('core_gameplay_open');
    else if(/story\.html/.test(href))track('story_open');
  },{passive:true});
})();